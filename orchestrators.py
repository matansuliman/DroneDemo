...
class Follow(BasicOrchestrator):
    ...
    def status(self):
        ...
        for model in self._models.values(): status += model.status()
        for controller in self._controllers.values(): status += controller.status()
        return status
    ...
    def _update_viewer_camera(self):
        ...
        self._viewer.cam.elevation = np.clip(-45 * np.linalg.norm(diff_pos) / 10, -45, -15)
    ...
    def _update_logs(self):
        for model in self._models.values(): model.update_log()
